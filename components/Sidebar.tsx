...
export const Sidebar = ({ activeView, setActiveView, onGoBack, userEmail, onLogout, onClose, isMobile }: SidebarProps) => {
    const shouldCloseOnAction = Boolean(isMobile && onClose);
        ...
        <aside className="w-64 bg-gray-800 p-4 border-r border-gray-700 flex flex-col h-full lg:h-screen flex-shrink-0">
            <div className="flex items-center mb-4 px-2 flex-shrink-0">
                ...
                {isMobile && onClose && (
                    <button
                        onClick={onClose}
                        className="ml-auto p-2 rounded-md text-gray-400 hover:text-white hover:bg-gray-700 transition-colors"
            ...
            <nav className="flex-grow space-y-2 overflow-y-auto pr-1">
                ...
                <NavItem icon={<TestGeneratorIcon />} label="Test Generator" isActive={activeView === 'tester'} onClick={() => handleSelectView('tester')} />
                <NavItem icon={<ComplexityIcon />} label="Complexity Analysis" isActive={activeView === 'complexity'} onClick={() => handleSelectView('complexity')} />
                <NavItem icon={<CommitIcon />} label="Commit Summarizer" isActive={activeView === 'summarizer'} onClick={() => handleSelectView('summarizer')} />
                <NavItem icon={<ArchitectureIcon />} label="Architecture Diagram" isActive={activeView === 'architecture'} onClick={() => handleSelectView('architecture')} />
                <NavItem icon={<ScaffoldIcon />} label="Project Scaffolder" isActive={activeView === 'scaffolder'} onClick={() => handleSelectView('scaffolder')} />
                <NavItem icon={<RunReviewIcon />} label="Run & Review" isActive={activeView === 'run_review'} onClick={() => handleSelectView('run_review')} />
                <NavItem icon={<SnippetIcon />} label="Snippet Library" isActive={activeView === 'snippets'} onClick={() => handleSelectView('snippets')} />
                ...
                <NavItem icon={<SimulationIcon />} label="Agent Simulation" isActive={activeView === 'simulation'} onClick={() => handleSelectView('simulation')} />
